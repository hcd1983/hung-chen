...
var _vegas = __webpack_require__(0);
...
$(function () {
	$(".vegas-banner").vegas({
		...
		init: function init(globalSettings) {
			var slides = $(".contact-banner").vegas('options', 'slides').length;
			while (slides--) {
	...
	$(".productsList .note").each(function (i, e) {
		$(e).html($(e).text().replace(/(\S)/g, '<span>$1</span>'));
	});

	$(".productsList").each(function (i, el) {

		var _h = 0;
		var _p = device == 'desktop' ? 204 : 180;

		$("li", el).each(function (i, el) {
